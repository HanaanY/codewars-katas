...
const {mean, variance, parseData} = require("../rainfall");
...
function assertFuzzyEquals(actual, expected, msg){
  ...
  assert.deepStrictEqual(inrange, true, msg || "abs(actual - expected) must be <= 1e-2. Expected was " + expected.toExponential(2) +", but got " + actual.toExponential(2));
}
...
describe("parseData", function() {
  it("parses a rainfall data string into a map", function() {
    assert.deepStrictEqual(parseData(data).get("Lima")[3], 0.4);
  })
})